...
<template>
    <header class="header-height flex items-center w-full bg-primaryColor text-white">
        <div :class="{ 'sidebar-width': true, 'sidebar-collapsed': isCollapsed }" class="w-full flex-shrink-0">
            <Link :href="route('dashboard.index')" :class="{ hidden: isCollapsed }"
                class="text-2xl px-4 font-bold text-nowrap">Dentisted</Link>
        </div>
                ...
                <Menu as="div" class="relative pt-1" v-slot="{ open }">
                    <MenuButton class="w-auto h-full inline-flex items-center gap-1">
                        <img class="w-10 h-10 object-cover rounded-full"
                            src="https://i.pinimg.com/564x/2e/51/1d/2e511db66f1b3f4c245dabf90b6e351d.jpg" alt="" />
