...
<template>
    <ul>
        <li  class="menu-item" :key="index">
            <div v-if="items" role="button" class="flex items-center gap-3 select-none px-3 py-2 hover:bg-lightBlueHover" @click="toggleSubMenu(index)">
            ...
            >
                <ul v-show="activeIndex === index" class="sub-menu ps-8 pb-0">
                    <SubMenuItem v-for="(item , subIndex) in items"
                                 :title="item.title"
                                 :href="item.href"
                                 :sub-index="subIndex"
                    />
